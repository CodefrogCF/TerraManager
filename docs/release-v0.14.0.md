...
```markdown
## TerraManager v0.14.0 – Pre-1.0 UX Polish

TerraManager v0.14.0 polishes the most frequently used Animal and Box workflows
and establishes the final feature baseline before the v1.0.0 MVP release.

### Highlights

- Use clear, localized Animal sex and birth-date-accuracy labels.
- Add a new Animal directly from an empty or populated Box detail page.
- Keep the originating Box preselected while retaining normal reassignment.
- Sort Boxes by creation time or natural Box number.
- Sort Animals by creation time, displayed name, age or latest feeding.
- Preserve the selected Box and Animal order across application restarts.
- Keep contextual detail navigation aligned with the visible sorted order.
- Preserve both sort preferences in portable backups.
- Use all new controls in English or German.
...
- Older backups remain restorable and use documented sorting defaults.
...
Commit and push the finalized release files before creating the tag:

```text
git add pubspec.yaml CHANGELOG.md README.md docs
git commit -m "release: prepare TerraManager v0.14.0"
