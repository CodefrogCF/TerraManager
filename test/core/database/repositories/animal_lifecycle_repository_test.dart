...
void main() {
  ...
  test('new animal is active by default', () async {
    final boxId = await createBox(
      'TM:BOX:11111111-1111-4111-8111-111111111111',
    );

    final animalId = await createAnimal(boxId);
  ...
  test('archive removes animal from box and preserves record', () async {
    final boxId = await createBox(
    ...
    final archivedAt = DateTime(2026, 9, 1, 12, 0);
    ...
    expect(animal!.status, AnimalStatus.archived);

    expect(animal.boxId, isNull);

    expect(animal.archiveReason, AnimalArchiveReason.sold);

    expect(animal.archivedAt, archivedAt);

    expect(animal.archiveNotes, 'Transferred to new owner');
  });

  test('archive trims archive notes', () async {
    final boxId = await createBox(
    ...
    expect(animal!.archiveNotes, 'Example note');
  });

  test('empty archive note is stored as null', () async {
    final boxId = await createBox(
  ...
  test('archived animal is excluded from box animals', () async {
    final boxId = await createBox(
    ...
    expect(await animalRepository.getAnimalsForBox(boxId), hasLength(1));
  ...
  test('active and archived queries return separate animals', () async {
    ...
    await createAnimal(boxId, commonName: 'Active Animal');

    final archivedId = await createAnimal(boxId, commonName: 'Archived Animal');
    ...
    expect(active.single.commonName, 'Active Animal');

    expect(archived, hasLength(1));

    expect(archived.single.commonName, 'Archived Animal');
  });

  test('restore assigns archived animal to selected box', () async {
    ...
    final animalId = await createAnimal(firstBoxId);
  ...
  test('restore fails when target box does not exist', () async {
    final boxId = await createBox(
    ...
    expect(
      () => animalRepository.restoreAnimal(animalId: animalId, boxId: 999999),
      throwsArgumentError,
    );
  });

  test('normal update cannot modify archived animal', () async {
    final boxId = await createBox(
      'TM:BOX:aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
    );

    final animalId = await createAnimal(boxId, commonName: 'Original Name');
    ...
    expect(animal.commonName, 'Original Name');
  });

  test('archived animal cannot be archived again', () async {
    final boxId = await createBox(
  ...
  test('active animal cannot be restored', () async {
    final firstBoxId = await createBox(
    ...
    expect(animal.boxId, firstBoxId);
  });
  ...
  test(
    ...
    () async {
      ...
      final animalId = await createAnimal(boxId, commonName: 'Archived Animal');
      ...
      await database
          ...
          .insert(
            FeedingEventsCompanion.insert(
              animalId: animalId,
              fedAt: DateTime(2026, 9, 1, 18, 0),
              notes: const drift.Value('Existing feeding'),
            ),
          );
      ...
      final deleted = await animalRepository.permanentlyDeleteArchivedAnimal(
        animalId,
      );
      ...
      final feedings = await (database.select(
        database.feedingEvents,
      )..where((event) => event.animalId.equals(animalId))).get();
  ...
  test(
    ...
    () async {
      ...
      final animalId = await createAnimal(boxId, commonName: 'Active Animal');
      ...
      await database
          ...
          .insert(
            FeedingEventsCompanion.insert(
              animalId: animalId,
              fedAt: DateTime(2026, 9, 1, 18, 0),
              notes: const drift.Value('Must survive'),
            ),
          );
