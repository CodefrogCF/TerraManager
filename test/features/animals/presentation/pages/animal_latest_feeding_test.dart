...
void main() {
  ...
  Future<void> scrollToLatestFeeding(WidgetTester tester) async {
    await tester.scrollUntilVisible(
      find.byKey(const Key('latest-feeding-heading')),
      300,
  ...
  testWidgets('shows empty latest feeding state', (tester) async {
    ...
    expect(find.byKey(const Key('latest-feeding-heading')), findsOneWidget);
  ...
  testWidgets('shows newest feeding date and note', (tester) async {
    final animalId = await createAnimal();
    ...
    expect(find.text('Latest mouse'), findsOneWidget);

    expect(find.text('Older feeding'), findsNothing);
  });

  testWidgets('archived animal still shows latest feeding', (tester) async {
    final animalId = await createAnimal();

    await feedingRepository.addFeeding(
      animalId,
      DateTime(2026, 8, 25, 19, 15),
    ...
    expect(find.text('25.08.2026 19:15'), findsOneWidget);

    expect(find.text('Before archive'), findsOneWidget);
  });

  testWidgets('latest feeding refreshes after returning from feeding history', (
    tester,
  ) async {
    final animalId = await createAnimal();
