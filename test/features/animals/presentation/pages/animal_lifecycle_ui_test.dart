...
void main() {
  ...
  testWidgets('active animal can be archived', (tester) async {
    ...
    final animalId = await createAnimal(boxId: boxId);

    await pumpDetail(tester, animalId: animalId);
    ...
    await tester.tap(find.text('Sold').last);

    await tester.pumpAndSettle();

    await tester.enterText(
      find.byKey(const Key('archive-notes-field')),
    ...
    expect(animal.archivedAt, isNotNull);
    ...
    expect(find.text('Transferred to new owner'), findsOneWidget);

    expect(find.byKey(const Key('restore-animal-button')), findsOneWidget);

    expect(find.byKey(const Key('edit-animal-button')), findsNothing);
  });

  testWidgets('archived animal can be restored to another box', (tester) async {
    ...
    final animalId = await createAnimal(boxId: firstBoxId);
    ...
    expect(
      find.text('TM:BOX:22222222-2222-4222-8222-222222222222'),
      findsNothing,
    );

    expect(
      find.text('TM:BOX:33333333-3333-4333-8333-333333333333'),
      findsNothing,
    );
  ...
  testWidgets('animal overview shows active animals only', (tester) async {
    ...
    await createAnimal(boxId: boxId, commonName: 'Active Animal');
    ...
    expect(find.text('Active Animal'), findsOneWidget);

    expect(find.text('Archived Animal'), findsNothing);
  });

  testWidgets('animal overview refreshes after animal is archived', (
    tester,
  ) async {
    final boxId = await createBox(
    ...
    expect(find.text('Corn Snake'), findsNothing);

    expect(find.text('No animals available'), findsOneWidget);
  });

  testWidgets('archived animal cannot be opened for editing', (tester) async {
    final boxId = await createBox(
  ...
  testWidgets('restore explains when no boxes are available', (tester) async {
    final boxId = await createBox(
    ...
    expect(
      find.text('Create a box before restoring this animal.'),
      findsOneWidget,
    );
  });
}
