...
void main() {
  ...
  Future<void> pumpPageWithNavigation(WidgetTester tester) async {
    ...
    await tester.tap(find.byKey(const Key('open-new-animal-button')));
  ...
  testWidgets('shows form when boxes are available', (tester) async {
    await createTestBox();
    ...
    expect(find.byKey(const Key('temp-max-field')), findsOneWidget);
  ...
  testWidgets('shows message when no boxes exist', (tester) async {
    ...
    expect(
      find.text('No boxes available. Create a box before adding an animal.'),
      findsOneWidget,
    );
  ...
  testWidgets('validates numeric fields', (tester) async {
    ...
    await tester.enterText(find.byKey(const Key('temp-min-field')), 'invalid');
  ...
  testWidgets('can select associated box', (tester) async {
    await createTestBox(qrId: 'test-box-001');
    ...
    expect(animals.single.boxId, secondBoxId);
  });
  ...
  testWidgets('returns to previous page after successful creation', (
    ...
    expect(find.text('New Animal'), findsNothing);

    expect(find.text('Open New Animal'), findsOneWidget);
  });

  testWidgets('can create animal with notes', (tester) async {
    await createTestBox();
    ...
    await tester.enterText(notesField, 'Calm animal, feeds well.');
    ...
    expect(animals.single.notes, 'Calm animal, feeds well.');
  });

  testWidgets('stores empty notes as null', (tester) async {
    await createTestBox();
