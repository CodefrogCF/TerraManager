...
void main() {
  ...
  testWidgets(
    'shows error for invalid QR code',
    (tester) async {
      ...
      expect(
        find.text('Invalid TerraManager QR code'),
        findsOneWidget,
      );

      expect(
        find.byKey(
          const Key('scanner-error'),
        ),
        findsOneWidget,
      );
    },
  ...
  testWidgets(
    'shows error when box does not exist',
    (tester) async {
      late Future<void> Function(String) scan;
  ...
  testWidgets(
    'opens box detail for known QR code',
    (tester) async {
      const qrId =
          'TM:BOX:12345678-1234-4123-8123-123456789abc';
